 ...
 *
 * R2 exposes an S3-compatible API authenticated with AWS Signature V4. Requests
 * use path-style addressing against the account endpoint
 * (`https://<account_id>.r2.cloudflarestorage.com`) and the fixed `auto` region.
 * Public delivery is served from a custom domain or the managed `r2.dev` domain,
 * so a public base URL should be configured for public buckets.
 *
...
class CloudflareR2Adapter extends AbstractAdapter {
	...
	public function test_connection(): array {
		...
		if ( ! $put ) {
			return [
				'success' => false,
				'message' => __( 'Upload test failed. Check the account endpoint, access key, secret key, and bucket name.', 'space-core' ),
			];
		}
	...
	private function region(): string {
		$region = trim( (string) ( $this->settings['region'] ?? '' ) );

		return '' !== $region ? $region : self::REGION;
	}
