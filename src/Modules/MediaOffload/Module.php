...
class Module extends AbstractModule {
	...
	public function boot(): void {
		...
		if ( ! empty( $options['sanitize_output'] ) ) {
			...
			if ( class_exists( 'WooCommerce' ) ) {
				...
				add_filter( 'woocommerce_product_get_short_description', [ $this, 'filter_sanitize_broken_urls_output' ], 9999, 2 );
			}
		}
	}
	...
	private function sanitize_settings( mixed $raw, bool $preserve_blank_secrets = true ): array {
		...
		$adapter  = in_array( (string) ( $raw['adapter'] ?? '' ), [ 'bunny', 'do_spaces' ], true ) ? (string) $raw['adapter'] : 'bunny';
		...
		$visibility       = in_array( (string) ( $raw['visibility'] ?? '' ), [ 'public', 'private' ], true ) ? (string) $raw['visibility'] : 'public';
	...
	public function ajax_save_settings(): void {
		...
		update_option( self::OPTION_KEY, $this->sanitize_settings( $data, true ) );
	...
	private function execute_migration_logic( string $old_base_url, string $new_base_url ): void {
		...
		foreach ( $post_rows as $row ) {
			...
			if ( $changed ) {
				++$posts_updated;
				if ( ! $dry_run ) {
					$wpdb->update( $wpdb->posts, [ 'post_content' => $new_content, 'post_excerpt' => $new_excerpt ], [ 'ID' => $id ], [ '%s', '%s' ], [ '%d' ] );
				}
			}
		}

		$meta_likes = [ '%' . $wpdb->esc_like( 'wp-content/uploads' ) . '%', '%' . $wpdb->esc_like( 'wp-content\/uploads' ) . '%' ];
	...
	private function execute_fix_broken_urls_logic(): void {
		...
		foreach ( $post_rows as $row ) {
			...
			if ( $changed ) {
				...
				if ( ! $dry_run ) {
					$res = $wpdb->update( $wpdb->posts, [ 'post_content' => $new_content, 'post_excerpt' => $new_excerpt ], [ 'ID' => $id ], [ '%s', '%s' ], [ '%d' ] );
					if ( false === $res ) {
						++$failures;
						if ( count( $failure_samples ) < $failure_samples_max && '' !== $wpdb->last_error ) {
							$failure_samples[] = [ 'table' => 'posts', 'id' => $id, 'error' => (string) $wpdb->last_error ];
						}
					}
				}
			}
			...
			if ( $changed ) {
				...
				if ( ! $dry_run ) {
					...
					if ( false === $res ) {
						++$failures;
						if ( count( $failure_samples ) < $failure_samples_max && '' !== $wpdb->last_error ) {
							$failure_samples[] = [ 'table' => 'postmeta', 'id' => $meta_id, 'error' => (string) $wpdb->last_error ];
						}
					}
				}
			}
			...
			if ( $changed ) {
				...
				if ( ! $dry_run ) {
					...
					if ( false === $res ) {
						++$failures;
						if ( count( $failure_samples ) < $failure_samples_max && '' !== $wpdb->last_error ) {
							$failure_samples[] = [ 'table' => 'options', 'id' => $option_id, 'error' => (string) $wpdb->last_error ];
						}
					}
				}
			}
			...
			if ( $changed ) {
				...
				if ( ! $dry_run ) {
					...
					if ( false === $res ) {
						++$failures;
						if ( count( $failure_samples ) < $failure_samples_max && '' !== $wpdb->last_error ) {
							$failure_samples[] = [ 'table' => 'term_taxonomy', 'id' => $tt_id, 'error' => (string) $wpdb->last_error ];
						}
					}
				}
			}
				...
				if ( $changed ) {
					...
					if ( ! $dry_run ) {
						...
						if ( false === $res ) {
							++$failures;
							if ( count( $failure_samples ) < $failure_samples_max && '' !== $wpdb->last_error ) {
								$failure_samples[] = [ 'table' => 'termmeta', 'id' => $meta_id, 'error' => (string) $wpdb->last_error ];
							}
						}
					}
				}
	...
	private function build_replacement_pairs( string $old_base_url, string $new_base_url ): array {
		...
		foreach ( $candidates as $old ) {
			...
			$pairs[] = [ 'search' => str_replace( '/', '\/', $old ), 'replace' => str_replace( '/', '\/', $new_base_url ) ];
		}
