...
namespace Space\Core\Modules\AdminCleaner;
...
/**
 * Admin Cleaner — two separate submenu pages:
...
class Module extends AbstractModule {
    ...
    public function get_label(): string {
        return __( 'Admin Cleaner', 'space-core' );
    }

    public function get_description(): string {
        return __( 'Hide / reorder dashboard widgets and admin menu items. Promote submenus to top-level links.', 'space-core' );
    }
    ...
    public function boot(): void {
        ...
        add_action( 'wp_ajax_sc_save_admin_cleaner', [ $this, 'ajax_save' ] );
    }

    private function get_options(): array {
        $saved = get_option( 'space_core_admin_cleaner', [] );
    ...
    public function enqueue_assets( string $hook ): void {
        if ( ! in_array( $hook, [ 'space-core_page_sc-admin-widgets', 'space-core_page_sc-admin-menu' ], true ) ) return;
        wp_enqueue_script( 'space-core-admin', SPACE_CORE_URL . 'assets/js/admin.js', [ 'jquery', 'jquery-ui-sortable' ], SPACE_CORE_VERSION, true );
    ...
    public function ajax_save(): void {
        ...
        if ( array_key_exists( 'hidden_widgets', $data ) ) {
            $current['hidden_widgets'] = array_map( 'sanitize_key', (array) $data['hidden_widgets'] );
        }
        ...
        update_option( 'space_core_admin_cleaner', $current );
    ...
    public function render_widgets_page(): void {
        ...
        $hidden_widgets  = $o['hidden_widgets'] ?? [];
        $nonce           = wp_create_nonce( 'space_core_admin' );
        ?>
        <div class="wrap sc-wrap">
            ...
            <div class="sc-tab-content" style="border-top:1px solid #c3c4c7;margin-top:16px;">
                ...
                <?php if ( empty( $widget_snapshot ) ) : ?>
                    <div class="notice notice-info inline"><p><?php esc_html_e( 'Visit the WordPress Dashboard once — widgets will appear here automatically.', 'space-core' ); ?></p></div>
                <?php else : ?>
                    <ul class="sc-cleaner-list" style="max-width:600px;">
                        <?php foreach ( $widget_snapshot as $id => $title ) : ?>
                            <li>
                                <label>
                                    <input type="checkbox" class="sc-hidden-widget" value="<?php echo esc_attr( $id ); ?>"
        ...
        jQuery(function($){
            $('#sc-widgets-save').on('click', function(){
                var $btn = $(this);
                var hiddenWidgets = [];
                $('.sc-hidden-widget:checked').each(function(){ hiddenWidgets.push($(this).val()); });
                $btn.prop('disabled', true);
                $.post(spaceCore.ajaxUrl, {
                    action: 'sc_save_admin_cleaner',
                    nonce:  spaceCore.nonce,
                    data:   JSON.stringify({ hidden_widgets: hiddenWidgets }),
                }, function(res){
                    ...
                    setTimeout(function(){ $('#sc-widgets-status').text(''); }, 3000);
    ...
    public function render_menu_page(): void {
                        ...
                        <?php foreach ( $menu_snapshot as $item ) :
                            ...
                            <li class="sc-menu-item<?php echo $is_hidden ? ' sc-item-hidden' : ''; ?>"
                                ...
                                <label class="sc-menu-label">
                                    <input type="checkbox" class="sc-hidden-menu" value="<?php echo esc_attr( $slug ); ?>"
                                           <?php checked( $is_hidden ); ?>>
                                    <?php echo esc_html( $label ); ?>
                                    <?php if ( $is_hidden ) : ?><span class="sc-hidden-badge"><?php esc_html_e( 'hidden', 'space-core' ); ?></span><?php endif; ?>
                                </label>
                                <?php if ( ! empty( $item['subs'] ) ) : ?>
                                    <ul class="sc-submenu-list">
                                        <?php foreach ( $item['subs'] as $sub ) :
                                            ...
                                            <li>
                                                <label>
                                                    <input type="checkbox" class="sc-hidden-submenu" value="<?php echo esc_attr( $key ); ?>"
        ...
        jQuery(function($){
            ...
            $('#sc-menu-save').on('click', function(){
                ...
                $('.sc-hidden-menu:checked').each(function(){ hiddenMenus.push($(this).val()); });
                $('.sc-hidden-submenu:checked').each(function(){ hiddenSubs.push($(this).val()); });
                $('#sc-menu-sortable .sc-menu-item').each(function(){ menuOrder.push($(this).data('slug')); });
                $('.sc-promote-sub.sc-promoted').each(function(){ promoted.push($(this).data('key')); });
                ...
                $.post(spaceCore.ajaxUrl, {
                    ...
                    $('#sc-menu-status').text(res.success ? '<?php echo esc_js( __( 'Saved!', 'space-core' ) ); ?>' : '<?php echo esc_js( __( 'Error.', 'space-core' ) ); ?>')
                        .css('color', res.success ? '#2e7d32' : '#c62828');
                    setTimeout(function(){ $('#sc-menu-status').text(''); }, 3000);
                }).always(function(){ $btn.prop('disabled', false); });
            });
        });
        </script>
