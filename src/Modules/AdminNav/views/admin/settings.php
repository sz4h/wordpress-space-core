    ...
    <table class="widefat" id="sc-nav-items" style="margin-top:12px;">
        ...
        <?php foreach ( $items as $item ) : ?>
            <tr class="sc-nav-row">
                ...
                <td><input type="text" class="sc-nav-label-en "
                           value="<?php echo esc_attr( is_array( $item['label'] ) ? ( $item['label']['en'] ?? '' ) : $item['label'] ); ?>"
                           placeholder="Label (EN)"></td>
                <td><input type="text" class="sc-nav-label-ar "
                           value="<?php echo esc_attr( is_array( $item['label'] ) ? ( $item['label']['ar'] ?? '' ) : '' ); ?>"
                           placeholder="اسم (AR)" dir="rtl"></td>
                <td><input type="text" class="sc-nav-url-inp "
                           value="<?php echo esc_attr( $item['url'] ); ?>"></td>
                <td><input type="text" class="sc-nav-icon-inp" value="<?php echo esc_attr( $item['icon'] ); ?>"
                           style="width:120px;"> <span class="sc-material-icon"
                                                       style="vertical-align:middle;font-family:'Material Symbols Outlined';font-size:20px;"><?php echo esc_html( $item['icon'] ); ?></span>
                </td>
                <td><input type="text" class="sc-nav-match-inp" value="<?php echo esc_attr( $item['match'] ?? '' ); ?>"
                           style="width:160px;"></td>
                <td>
                    <button type="button" class="button sc-nav-remove"
    ...
    jQuery(function ($) {
        ...
        $('#sc-nav-add').on('click', function () {
            var row = '<tr class="sc-nav-row">' +
                '<td><span class="dashicons dashicons-move" style="cursor:grab;color:#bbb;"></span></td>' +
                '<td><input type="text" class="sc-nav-label-en " value="" placeholder="Label (EN)"></td>' +
                '<td><input type="text" class="sc-nav-label-ar " value="" placeholder="اسم (AR)" dir="rtl"></td>' +
                '<td><input type="text" class="sc-nav-url-inp " value=""></td>' +
