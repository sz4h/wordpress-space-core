...
class AdminMenu {
    ...
    private const SELF_MANAGED = [ 'stats', 'local_shipping', 'guest_orders', 'admin_cleaner' ];
    ...
    public function register_menu(): void {
        ...
        add_submenu_page( 'space-core', __( 'Modules', 'space-core' ), __( 'Modules', 'space-core' ), 'manage_options', 'space-core', [ $this, 'render_page' ] );
        ...
        foreach ( $registry as $slug => $class ) {
            if ( ! in_array( $slug, $enabled, true ) ) continue;
            if ( in_array( $slug, self::SELF_MANAGED, true ) ) continue;

            $module = new $class( $slug );
            if ( ! method_exists( $module, 'render_settings' ) ) continue;
            ...
            add_submenu_page(
                ...
                'manage_options',
                $menu_slug,
                function() use ( $module ) { $this->render_module_settings_page( $module ); }
            );
        }
    ...
    public function enqueue_assets( string $hook ): void {
        $is_sc_page = 'toplevel_page_space-core' === $hook
            || str_starts_with( $hook, 'space-core_page_sc-' );

        if ( ! $is_sc_page ) return;
    ...
    public function ajax_import(): void {
        ...
        foreach ( $data as $key => $value ) {
            if ( ! str_starts_with( $key, 'space_core_' ) ) continue;
        ...
        <div class="wrap sc-wrap">
            ...
            <div class="sc-tab-content" style="border-top:1px solid #c3c4c7;margin-top:16px;">
                <div style="max-width:650px;padding-top:16px;">
                    ...
                    <p>
                        <input type="file" id="sc-tools-import-file" accept=".json" style="display:block;margin-bottom:10px;">
